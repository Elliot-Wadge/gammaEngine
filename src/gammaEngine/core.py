...
@njit
def _gamma_3D_pr(coord_eval: npt.NDArray[np.float64],
              ref_z_axes: npt.NDArray[np.float64], 
              ref_y_axes: npt.NDArray[np.float64], 
              ref_x_axes: npt.NDArray[np.float64], 
              dose_eval: npt.NDArray[np.float64], 
    ...
    res = np.zeros(dose_eval.shape, dtype=np.uint8)
...
@njit(parallel=True)
def _gamma_2D_pr(coord_eval: npt.NDArray[np.float64], 
              ref_y_axes: npt.NDArray[np.float64], 
              ref_x_axes: npt.NDArray[np.float64], 
              dose_eval: npt.NDArray[np.float64], 
              dose_ref: npt.NDArray[np.float64], 
    ...
    for i in prange(len(coord_eval)):
        ...
        k = 1
        brk = False
        while d < distance_threshold and not brk:
...
@njit(parallel=True)
def _gamma_1D_pr(x_eval:npt.NDArray[np.float64], 
              x_ref:npt.NDArray[np.float64], 
              dose_eval:npt.NDArray[np.float64], 
              dose_ref:npt.NDArray[np.float64], 
              dose_threshold:float, 
              ...
              interp_res:float, 
              norm:float):
    '''1D gamma that only computes pass or fail not the actual gamma score'''
    # precompute useful values
